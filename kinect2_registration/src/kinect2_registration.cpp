...
DepthRegistration *DepthRegistration::New(Method method)
{
  ...
  {
  case DEFAULT:
    std::cout<<"No default registration method available!"<<"\n";
    break;
  case CPU:
#ifdef DEPTH_REG_CPU
    std::cout<<"Using CPU registration method!"<<"\n";
    return new DepthRegistrationCPU();
#else
    std::cout<<"CPU registration method not available!"<<"\n";
    break;
#endif
  case OPENCL:
#ifdef DEPTH_REG_OPENCL
    std::cout<<"Using OpenCL registration method!"<<"\n";
    return new DepthRegistrationOpenCL();
#else
    std::cout<<"OpenCL registration method not available!"<<"\n";
    break;
#endif
  }
  return NULL;
